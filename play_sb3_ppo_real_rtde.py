...
csv_path = os.path.join(save_dir, "real_observations_predefined_pose.csv")
...
def sample_predefined_pose(current_index):
    ...
    return predefined_poses[current_index]
...
def run_on_real_robot():
    ...
    while True:
        ...
        distance = np.linalg.norm(target_pose[:3] - tcp_pose[:3])
        # time.sleep(1)
